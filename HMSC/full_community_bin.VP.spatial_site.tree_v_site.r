...
source("~/repo/neonectria_barcoding_012220/sum_trees/read_ASV_dat.LULU_tab.r") #when we use the version in the bbd_comm_comp repo the script breaks for some reason. Or it might be loqding tidyverse in this script instead of through the sourced script... Will have to test. Although, the version in the neonectria repo should not work because the paths will not correct (not absolute paths...)
